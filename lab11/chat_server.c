...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    usernames[user_index].username = malloc(sizeof(char) * 30);
    
    read(client_fd, usernames[user_index].username, 30 * sizeof(char));
    // printf("Username: %s \n ", usernames[user_index].username);
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    strcat(buf,": ");
    int username_size = strlen(usernames[client_index].username) + (2 * sizeof(char));
    char read_buf[BUF_SIZE +1 - username_size];
    
    int num_read = read(fd, &read_buf, BUF_SIZE);
    
    strcat(buf, read_buf);
    buf[num_read + username_size] = '\0';
    for(int i = 0; i < MAX_CONNECTIONS; i++) {
        int curr_fd = usernames[i].sock_fd;
        if (fd == curr_fd) {
            continue;
        }
        else if (num_read == 0 || write( curr_fd, buf,  strlen(buf)) != strlen(buf)) {
